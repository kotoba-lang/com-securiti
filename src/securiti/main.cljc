(ns securiti.main
  "Kotodama WASM entrypoint for the Securiti clean-room actor (L4) — Clojure port.
  ...
  facts (`securiti.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "securiti")
(def tier "L4")
...
(def entity-specs
  [{:entity "DataSubject" :plural "datasubjects" :id-prefix "securiti_dat"
    :fields [:email :jurisdiction :status] :required [:email :jurisdiction]
    :coerce {} :refs {}}
   {:entity "ConsentRecord" :plural "consentrecords" :id-prefix "securiti_con"
    :fields [:subjectId :purpose :granted :recordedAt] :required [:purpose :granted]
    :coerce {:granted :bool} :refs {}}
   {:entity "Request" :plural "requests" :id-prefix "securiti_req"
    :fields [:subjectId :type :status :dueAt] :required [:type :status]
    :coerce {} :refs {}}
   {:entity "Control" :plural "controls" :id-prefix "securiti_con"
    :fields [:framework :name :status] :required [:framework :name]
    :coerce {} :refs {}}
   {:entity "Evidence" :plural "evidences" :id-prefix "securiti_evi"
    :fields [:controlId :name :contentRef] :required [:name]
    :coerce {} :refs {:controlId "Control"}}
   {:entity "Assessment" :plural "assessments" :id-prefix "securiti_ass"
...
(defn emit-facts
  "EAVT facts for one record: {\"securiti.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "securiti-compat" :tier tier :entities entities} 200])
...
(defn start! [] :securiti-compat/ready)
